    ...
    return Promise.resolve().then(() => {
      ...
      return config.locator.findTool('go', options).then((cmd) => {
        ...
        return Promise.all([buildPromise, testPromise]).then((results) => {
          let messages = []
          let compareMessage = (msg1, msg2) => {
            return msg1.filePath === msg2.filePath && msg1.row === msg2.row &&
              msg1.text === msg2.text && JSON.stringify(msg1.range) === JSON.stringify(msg2.range)
          }
          for (let result of results) {
            for (let newMessage of result) {
              if (!messages.find((msg) => compareMessage(newMessage, msg))) {
  ...
  mapMessages (data, cwd, linterName) {
    ...
    let extract = (matchLine) => {
      ...
      if (matchLine[2] && matchLine[2] === '#') {
        ...
        messages.push({name: linterName, type: 'Error', row: row, column: column, text: text, filePath: file, range: range})
      }
    }
    let match
